...
class MarkItDownPipelineDemo:
    """RAGTool的MarkItDown处理管道演示类"""

    NAMESPACE = "markitdown_demo"
    CHUNK_SIZE = 300
    CHUNK_OVERLAP = 50

    def __init__(self):
        self.rag_tool = RAGTool(
            knowledge_base_path=os.path.join(_SCRIPT_DIR, "demo_rag_kb"),
            rag_namespace=self.NAMESPACE,
        )
        self.temp_dir = tempfile.mkdtemp()
        if not self.rag_tool.initialized:
            raise RuntimeError(
                f"RAG工具初始化失败: {getattr(self.rag_tool, 'init_error', '未知错误')}"
            )

    def _run(self, **params):
        """统一传入 namespace，与 rag_namespace 保持一致。"""
...
知识图谱,技术组件,0.9,实体关系的结构化表示"""
    ...
    def demonstrate_markitdown_conversion(self, file_paths):
        ...
        for filename, path in file_paths.items():
            ...
            # 使用 RAGTool 添加文档，内部会调用 MarkItDown
            result = self._run(action="add_document", file_path=path)
    ...
    def demonstrate_markdown_chunking(self):
        ...
        print(f"• 🔢 Token精确控制 - chunk_size={self.CHUNK_SIZE}")
        print(f"• 🔗 智能重叠策略 - chunk_overlap={self.CHUNK_OVERLAP}")
...
"""
        ...
        for query, description in search_queries:
            ...
            search_result = self._run(action="search", query=query, limit=2)
...
"""
    ...
    def demonstrate_pipeline_performance(self):
        ...
        stats = self._run(action="stats")
        print(f"\n📊 处理后统计: {stats}")


def _print_conversion_summary(conversion_results):
    """输出格式转换汇总。"""
    print("\n📋 格式转换汇总:")
    for filename, info in conversion_results.items():
        status = "成功" if "✅" in info["result"] else "失败"
        print(f"  • {filename}: {status} ({info['time']:.3f}秒)")
...
def main():
    ...
    try:
        ...
        if demo and os.path.isdir(demo.temp_dir):
